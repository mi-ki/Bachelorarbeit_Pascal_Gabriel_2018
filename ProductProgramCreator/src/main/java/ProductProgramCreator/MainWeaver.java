...
public class MainWeaver {
     ...
     * @param weaveAuto true means automated method weaving, false only if annotated
     * @throws InstantiationException When a rule instantion has failed
     * @throws IllegalAccessException  if a rule or its nullary constructor is not available
     * @throws FileNotFoundException if the given file could not be found
     */
    public MainWeaver(String addL, String addR, String mainMethodLeft, String mainMethodRight, String path, boolean weaveAuto)
     ...
     * @throws InstantiationException When a rule instantion has failed
     * @throws IllegalAccessException  if a rule or its nullary constructor is not available
     * @throws IOException if an I/O-Exception has occurred or the ressource could not be found.
     */
    public MainWeaver(String addL, String addR, String mainMethodLeft, String mainMethodRight)
        ...
        this.allDeclaredMethods = new LinkedList();
    }
    /**
     * A constructor which defines only the suffixes for the inputprograms.
    ...
    /**
     * The default constructor, assumes the left suffix is '_1' and the right suffix is '_2'.
     * Defaults the main method names to 'main'.
     * Defaults the priority list.
     * Collects all weaving rules present
     * @throws InstantiationException When a rule instantion has failed
     ...
     * @throws IOException if an I/O-Exception has occurred or the ressource could not be found.
     */
    public MainWeaver() throws InstantiationException, IllegalAccessException, IOException {
        this.additionL = "_1";
        this.additionR = "_2";
        this.mainMethodLName = "main";
        this.mainMethodRName = "main";
     ...
     */
    public CompilationUnit initiateWeave(CompilationUnit leftIn, CompilationUnit rightIn)
        ...
        //Extract the names of the main classes and remove ".java", add the suffix from the renamingprocess
        String leftMainClassName = left.getStorage().get().getFileName().toString();
        leftMainClassName = leftMainClassName.substring(0, leftMainClassName.length() - 5) + this.additionL;
        String rightMainClassName = right.getStorage().get().getFileName().toString();
        rightMainClassName = rightMainClassName.substring(0, rightMainClassName.length() - 5) + this.additionR;
        ...
        //Add all classes(including main classes, since self instantiation is possible)
        ...
        ClassOrInterfaceDeclaration retMainClass = retCu.getClassByName(leftMainClassName + "_x_" + rightMainClassName).get();
        ClassOrInterfaceDeclaration leftMainClass = left.getClassByName(leftMainClassName).get();
        ClassOrInterfaceDeclaration rightMainClass = right.getClassByName(rightMainClassName).get();
        ...
        //adds all members except and the main method to the product classes body, and extract the main methods
        MethodDeclaration mainMethodL = addMembersAndReturnMainMethod(retMainClass, leftMainClass, mainMethodLName + additionL);
        MethodDeclaration mainMethodR = addMembersAndReturnMainMethod(retMainClass, rightMainClass, mainMethodRName + additionR);
        MethodDeclaration retMainMethod = new MethodDeclaration();

        //Throw exception if mainMethods could not be found
        //this should never be the case, as the presence of the methods are checked
        //when their name is given by the user
        ...
        if(mainMethodL.isStatic() && !mainMethodR.isStatic()) {
            String exMessage = "Either both classes or no class can be static.";
            exMessage = exMessage + "\nWeaving aborted.";
            throw new WeaveException(exMessage);
        ...
        //Checks if the main methods have return types, transforms it to a class without return type
        //Return values will be saved in a field
        String nameOfReturnFieldLeft = "retValueOf" + mainMethodLName + additionL;
        //FieldDeclaration retFieldLeft = transformToVoid(leftMainClass, mainMethodL,nameOfReturnFieldLeft);
        MethodRule helper = new MethodRule();
        FieldDeclaration retFieldLeft = helper.removeReturn(mainMethodL);
        if(retFieldLeft != null) {
            //retMainClass.addField(retFieldLeft.getElementType(), nameOfReturnFieldLeft, Modifier.PUBLIC);
            addDeclarationToMainClass(retFieldLeft);
        }
        String nameOfReturnFieldRight = "retValueOf" + mainMethodRName + additionR;
        //FieldDeclaration retFieldRight = transformToVoid(rightMainClass, mainMethodR, nameOfReturnFieldRight);
        FieldDeclaration retFieldRight = helper.removeReturn(mainMethodR);
        if(retFieldRight != null) {
            //retMainClass.addField(retFieldRight.getElementType(), nameOfReturnFieldRight, Modifier.PUBLIC);
            addDeclarationToMainClass(retFieldRight);
        }

        //set the type to void, since all return values will be redirected to fields
        ...
        //Call the weave function and add the main method to the return lass
     ...
     */
    public BlockStmt weave(BlockStmt left, BlockStmt right) throws WeaveException {
        ...
        while(leftHasNext || rightHasNext) {
            ...
            if(leftHasNext && rightHasNext) {
                ...
                //check if one of the current nodes has a weave annotation in its comment
                leftAtAnnotation = hasWeaveAnnotation(currentLeft);
                rightAtAnnotation = hasWeaveAnnotation(currentRight);
                //make sure if there is one side at the start of the annotation,
                //the second will be too
                boolean canCollectStatements = false;

                if(leftAtAnnotation && rightAtAnnotation) {
                    boolean sameLabel = getLabelOfAnno(currentLeft).equals(getLabelOfAnno(currentRight));
                    boolean sameType = getTypeOfAnno(currentLeft).equals(getTypeOfAnno(currentRight));
                    canCollectStatements = sameLabel && sameType;
                } else if(leftAtAnnotation) {
                    while(!rightAtAnnotation && rightHasNext) {
                        //Add all statements in front of the annotation to the block
                        retBlock.addStatement(currentRight);
                        currentRight = rightIterator.next();
                        rightHasNext = rightIterator.hasNext();
                        //set it to true ONLY if both have the exact same annotation
                    ...
                    if(rightAtAnnotation) {
                        ...
                        String anno = getTypeOfAnno(currentLeft) + " " + getLabelOfAnno(currentLeft);
                        String exMessage = "2nd Annotation for " + anno + " not found.\nWeaving aborted.";
                        throw new WeaveException(exMessage);
                    }
                    ...
                    if(leftAtAnnotation) {
                        ...
                        String anno = getTypeOfAnno(currentRight) + " " + getLabelOfAnno(currentRight);
                        String exMessage = "2nd Annotation for " + anno + " not found.\nWeaving aborted.";
                        throw new WeaveException(exMessage);
                    }
                ...
                if(canCollectStatements) {
                    //At the start of both annotated areas, now collect all statements
                    ...
                    while(leftHasNext && !leftFinished) {
                        ...
                        if(hasWeaveAnnotation(currentLeft)) {
                            isCorrectLabel = getLabelOfAnno(currentLeft).equals(label);
                        } else {
                            leftArea.addStatement(currentLeft);
                            leftHasNext = leftIterator.hasNext();
                        }
                    ...
                    while(rightHasNext && !rightFinished) {
                        ...
                        boolean isCorrectLabel = false;
                        if(hasWeaveAnnotation(currentRight)) {
                            isCorrectLabel = getLabelOfAnno(currentRight).equals(label);
                        } else {
                            rightArea.addStatement(currentRight);
                            rightHasNext = rightIterator.hasNext();
                        }
                    ...
                    WeavingRule currentRule = weavingRules.get("AnnotationRule");
                    for (Statement current : currentRule.weave(leftArea, rightArea)) {
                ...
                if(!annoJustFinished) {
                    ...
                    boolean leftHasRule = weavingRules.containsKey(currentLName);
                    boolean rightHasRule = weavingRules.containsKey(currentRName);
                    if (leftHasRule && rightHasRule) {
                        boolean leftHigher = priorityList.hasHigherPriority(currentLName, currentRName);
                        //Check if the right statement has the same type
                        if(currentLName.equals(currentRName)) {
                            stopLeft = false;
                            stopRight = false;
                            //weave them and add the result
                            WeavingRule currentRule = weavingRules.get(currentLName);
                            LinkedList<Statement> allStmts = currentRule.weave(currentLeft, currentRight);
                            for (Statement current : allStmts) {
                                retBlock.addStatement(current);
                            }
                        } else if(leftHigher) {
                            //add the right rule and stop searching on the left side
                            stopLeft = true;
                            stopRight = false;
                            retBlock.addStatement(currentRight);
                        } else if(!leftHigher) {
                            //add the left rule and stop searching on the right side
                            stopLeft = false;
                            stopRight = true;
                            retBlock.addStatement(currentLeft);
                        } else {
                            //This case will happen if both rules aren't in the prio list
                ...
                //If left blocks but no more right elements, then remove block and add all
                if(stopLeft) {
                    stopLeft = false;
                    retBlock.addStatement(currentLeft);
                }
                ...
                currentLName = checkExpressionStmt(currentLeft);
                //check for the case, that the right one still searches for a match
                if(stopRight) {
                    ...
                    if(currentLName.equals(currentRName)) {
                        ...
                        //weave them and add the result
                        WeavingRule currentRule = weavingRules.get(currentLName);
                        for (Statement current : currentRule.weave(currentLeft, currentRight)) {
                            retBlock.addStatement(current);
                        }
                    } else {
                        retBlock.addStatement(currentLeft);
                    ...
                    //no search ongoing, no more elements right, so add all remaining left
                    retBlock.addStatement(currentLeft);
                }
            } else if (rightHasNext) {
                //No need for annotationhandling, since annotations will only
                //block inside the first if-branch
                //If right blocks but no more left elements, then remove block and add all
                if(stopRight) {
                    stopRight = false;
                    retBlock.addStatement(currentRight);
                }
                ...
                currentRName = checkExpressionStmt(currentRight);
                //check for the case, that the left one still searches for a match
                if(stopLeft) {
                    //if they are the same type, weave them
                    if(currentLName.equals(currentRName)) {
                        stopLeft = false;
                    ...
                    //no search ongoing, no more elements on the left, so add all remaining right
                    retBlock.addStatement(currentRight);
                }
            }
    ...
    /**
     * Sets the type of methode weaving to be used.
     * true means automated weaving(as with any other weaving rule, but has lowest priority)
    ...
    private void addImports(CompilationUnit target, CompilationUnit source) {
        for(ImportDeclaration current : source.getImports()) {
            target.addImport(current.getNameAsString(), current.isStatic(), current.isAsterisk());
        }
    }
     ...
     * @param source
     */
    private void addAllClasses(CompilationUnit target, CompilationUnit source) {
    ...
    /**
     * Returns the modifiers of a given class without public, private, protected.
     * Checks for unsupported modifiers
     * @param inputClass
     * @return  the modifiers
     */
    private EnumSet<Modifier> checkModifiers(ClassOrInterfaceDeclaration inputClass)
                                                            throws WeaveException{
        ...
        if(inputClass.isPublic()) {
            ...
            System.out.println("The class " + inputClass.getNameAsString() +
                    " is not public. The product class will be set to public.");
        }
        ...
        LinkedList<String> unsupportedModifiers = new LinkedList();
    ...
    /**
     * Returns the modifiers of a given method without public, private, protected.
     * Checks for unsupported modifiers
     * @param inputMethod
     * @return  the modifiers
     */
    private EnumSet<Modifier> checkModifiers(MethodDeclaration inputMethod)
                                                          throws WeaveException{
        ...
        if(inputMethod.isPublic()) {
            ...
            System.out.println("The method " + inputMethod.getNameAsString() +
                    " is not public. The product method will be set to public.");
        }
    ...
    /**
     * Adds all members of the source class to the target except the specified method.
     * @param target
     * @param source
     * @param nameWithAddition  The method to not add, with the correct modifier( that got added in the renaming process)
     * @return  The main Method, null if the main method could not be found
     */
    private MethodDeclaration addMembersAndReturnMainMethod(ClassOrInterfaceDeclaration target, ClassOrInterfaceDeclaration source, String nameWithAddition) {
        ...
        for(BodyDeclaration<?> current : nodes) {
            ...
            } else if(!current.asMethodDeclaration().getName().toString().equals(nameWithAddition)) {
                target.addMember(current.clone());
            } else {
    ...
    /**
     * Transforms the given method into a method without return value, adds a field to the given class
     * @param inputMethod
     * @param nameOfField The name the returned field should have
     * @return the declaration of the field that got added, null if method was already void
     */
    private  FieldDeclaration transformToVoid(ClassOrInterfaceDeclaration correspondingClass, MethodDeclaration inputMethod, String nameOfField) {
        FieldDeclaration retField = null;
        Type type = inputMethod.getType();
        if (!type.isVoidType()) {
            //add return value to corresponding class
            retField = correspondingClass.addField(type, nameOfField, Modifier.PUBLIC);
            for(Statement current : inputMethod.getBody().get().getStatements()) {
                if (current.isReturnStmt()) {
                    AssignExpr replacement = new AssignExpr();
                    replacement.setOperator(AssignExpr.Operator.ASSIGN);
                    replacement.setTarget(retField.getVariable(0).getNameAsExpression());
                    if(current.asReturnStmt().getExpression().isPresent()) {
                        replacement.setValue(current.asReturnStmt().getExpression().get());
                    } else {
                        //TODO Ausgabe von Exception, da vorausgesetzt wurde, das nur EIN return das ETWAS zurückgibt.
                    }
     ...
     * @param source
     */
    private void addParameters(MethodDeclaration target, MethodDeclaration source) {
        //Sets the parameters
    ...
    private HashMap<String, WeavingRule> getRules ()
        ...
        HashMap<String, WeavingRule> retMap = new HashMap();
        ...
        for(Class current : implementingClasses) {
            WeavingRule currentRule = (WeavingRule)current.newInstance();
    ...
    /**
     * This method is used for determining which name the given statement should
     * be given(for weaving purpose).
    ...
    private String checkExpressionStmt(Statement stmt) {
        ...
        if(stmt.isExpressionStmt() && methodWeavingAutomated) {
            ...
            if(expr.isAssignExpr() && weave) {
                ...
                //return regular name
                return stmt.asExpressionStmt().getExpression().getMetaModel().toString();
            }
        } else {
    ...
    /**
     * Checks wether the comments of a given statement start with '@weave'
     * @param in
    ...
    /**
     * Returns a list of names of all in either compilation unit declared methods.
     * @return
     */
    ...
    /**
     * Checks if the given statements has a '@methodWeave' annotation at the start
     * of its comments.
     ...
     */
    public HashMap<String, MethodDeclaration> getMethodAndClass (String methodName)
                                                        throws WeaveException{
        ...
        if(visitor.methodFound()) {
            ...
            throw new WeaveException("The method declaration of: " + methodName +
                    " could not be found.\nWeaving aborted");
        }
    }

    /**
     * Adds a given fielddeclaration to the body of the main class of the product
     * @param toAdd
     */
    ...
    /**
     * Adds a given name to the list of methods that are declared inside this CU
     * Is used for making sure, that a recursive method weaving does not loop
     * endlessly.
     * @param name
     */
     ...
     * Expects there to be an annotation. Will return an empty string otherwise.
     * @param in the statement to check on
    ...
    private String getLabelOfAnno(Statement in) {
        ...
        if(hasWeaveAnnotation(in)) {
            ...
            retLabel = retLabel.substring(retLabel.indexOf("["), retLabel.indexOf("]"));
        }
        return retLabel;
    }
}
